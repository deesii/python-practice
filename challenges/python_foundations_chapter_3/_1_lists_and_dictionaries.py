...
def lowest_squared(list_original):
    return min(list(map(lambda x : x**2 , list_original)))
...
def highest_squared(list_original):
    return max(list(map(lambda x : x**2 , list_original)))
...
def starts_with_a(list_original):
    return list(filter(lambda x : x.startswith("a"), list_original))
...
def starts_with_a_vowel(list_original):
    vowels = "aeiou"
    return [i for i in list_original if i[0] in vowels]
...
def reverse_each_element(list_original):
    return list(map(lambda x : x[::-1] , list_original))
...
def sort_by_last_letter(list_original):
    return sorted(list_original, key = lambda x : x[-1], reverse = False )
...
def greater_than_5(list_original):
    return [i for i in list_original if i > 5]
...
def all_above(list, num):
    return len([i for i in list if i <= num]) == 0
#   not any([i for i in list if i <= num]) also works
...
def all_below(list, num):
    return not any([i for i in list if i >= num])
...
def multiply_each_by(list_original , num):
    return list(map(lambda x : x*num, list_original))
...
def where_value_below(dictionary, num):
    return {key:value for (key,value) in dictionary.items() if value < num}
...
def where_key_starts_with(dictionary, letter):
    return {key:value for (key,value) in dictionary.items() if key.startswith(letter)}